...
export function FeedbackScreen({
  ...
  useEffect(() => {
    if (!correct) return;
    let cancelled = false;
    dataLayer.getConfig().then((config) => {
      if (cancelled || !config.auto_next_correct) return;
      const delayMs = config.auto_next_delay_ms;
      setAutoNextCountdown(delayMs);
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div
        className={`rounded-lg p-6 mb-4 ${correct ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
      >
        <h3
          className={`text-xl font-bold mb-3 ${correct ? "text-green-700" : "text-red-700"}`}
        >
        ...
        <p className={correct ? "text-green-700" : "text-gray-700"}>
          Correct answer:{" "}
          <span className="font-medium">{result.question.correct_option}</span>
        </p>
      </div>
      ...
      {autoNextCountdown !== null && (
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm text-gray-500">
            Next question in {Math.ceil(autoNextCountdown / 1000)}s...
          </span>
        </div>
      )}
